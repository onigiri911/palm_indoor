...
!>   - dimension values must be written by all MPI ranks later
...
!--------------------------------------------------------------------------------------------------!
 FUNCTION dom_def_dim( file_name, dimension_name, output_type, bounds,                             &
    ...
    LOGICAL, INTENT(IN), OPTIONAL ::  write_only_by_master_rank  !< true if only master rank shall write this variable
    ...
    DO  f = 1, nfiles
       ...
       IF ( files(f)%is_init )  THEN
          ...
          SELECT CASE ( TRIM( files(f)%format ) )
             ...
             CASE ( 'netcdf4-parallel', 'netcdf4-serial' )
                CALL netcdf4_finalize( TRIM( files(f)%format(9:) ), files(f)%id,                   &
                                       output_return_value )
...
!--------------------------------------------------------------------------------------------------!
 FUNCTION write_attribute( file_format, file_id, file_name,                                        &
                           variable_id, variable_name, attribute )                                 &
             RESULT( return_value )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE stop_file_header_definition( file_format, file_id, file_name, return_value )
    ...
    SELECT CASE ( TRIM( file_format ) )
       ...
       CASE ( 'netcdf4-parallel', 'netcdf4-serial' )
          CALL netcdf4_stop_file_header_definition( file_format(9:), file_id, output_return_value )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE dom_database_debug_output
    ...
    INTEGER, PARAMETER ::  max_keyname_length = 8  !< length of longest key name
...
!--------------------------------------------------------------------------------------------------!
    SUBROUTINE print_dimensions( indent_level, dimensions )
       ...
       INTEGER, PARAMETER ::  max_keyname_length = 26  !< length of longest key name
