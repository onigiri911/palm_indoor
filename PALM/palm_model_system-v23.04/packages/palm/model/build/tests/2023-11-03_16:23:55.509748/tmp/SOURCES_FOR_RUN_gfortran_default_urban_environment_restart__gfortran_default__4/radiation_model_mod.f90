...
!-- Localized raytracing IPC message; record for the whole raytraced angular section
    TYPE t_traced_section
       CHARACTER ::  message  !< Message code, uppercase=finalize the respective task:
                              !< f = SVF raytrace forward
                              !< b = SVF raytrace backward
...
!-- Allocate array for storing the surface net radiation
    IF ( .NOT. ALLOCATED( surf_lsm%rad_net )  .AND.  surf_lsm%ns > 0  )  THEN
       ALLOCATE( surf_lsm%rad_net(1:surf_lsm%ns) )
       surf_lsm%rad_net = 0.0_wp
    ENDIF
    IF ( .NOT. ALLOCATED( surf_usm%rad_net )  .AND.  surf_usm%ns > 0  )  THEN
       ALLOCATE( surf_usm%rad_net(1:surf_usm%ns) )
       surf_usm%rad_net = 0.0_wp
    ENDIF
!
!-- Allocate array for storing the surface longwave (out) radiation change
    IF ( .NOT. ALLOCATED( surf_lsm%rad_lw_out_change_0 )  .AND.  surf_lsm%ns > 0 )  THEN
       ALLOCATE( surf_lsm%rad_lw_out_change_0(1:surf_lsm%ns) )
       surf_lsm%rad_lw_out_change_0 = 0.0_wp
    ENDIF
    IF ( .NOT. ALLOCATED( surf_usm%rad_lw_out_change_0 )  .AND.  surf_usm%ns > 0 )  THEN
       ALLOCATE( surf_usm%rad_lw_out_change_0(1:surf_usm%ns) )
       surf_usm%rad_lw_out_change_0 = 0.0_wp
    ENDIF
!
!-- Allocate surface arrays for incoming/outgoing short/longwave radiation.
!-- For attributes that are part of the restart IO check if these have been already allocated.
...
!-- Save regular view-factor outputs for SVF and MRTF
    IF ( s%message == 'b'  .OR.  s%message == 'M' )  THEN
       DO  iray = 1, s%nrays
          nsvf_ins = nsvf_ins + 1
...
!-- Later for  for slanted surfaces, we will use generic zdir approach
    IF ( s%message == 'b'  .AND.  r(1)%zdir > 0.0_wp )  THEN
...
!--       checked if they are already obstructed.
          DO WHILE ( s%lowest_mixed_ray > s%lowest_free_ray )
             ...
             IF ( r(s%lowest_mixed_ray)%itarget < 0 )  THEN
                ...
                CALL process_ray_canopy( s%lowest_mixed_ray, .FALSE.,                              &
...
!--                oriented faces
                   DO  k = FLOOR( 0.5_wp + s%origin(1) + r(iray)%zdir * s%lastdist ),              &
                      ...
                      IF ( r(iray)%itarget >= 0 )  THEN
                         CALL process_ray_canopy( iray, .FALSE., REAL(k, wp) - 0.5_wp )
                         EXIT
                      ENDIF
                   ENDDO
                ELSE
...
!--       (no backward raytracing)
    ...
    ENDIF ! .NOT. backward
!
!-- Backward raytracing remote request
    DO
!
!--    Along what dimension will the next wall crossing be?
