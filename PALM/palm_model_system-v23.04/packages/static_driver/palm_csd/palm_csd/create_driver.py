...
from palm_csd.netcdf_data import NCDFDomain
...
def create_driver(input_configuration_file: str) -> None:
    ...
    # Create domains
    domain_dict: Dict[str, NCDFDomain] = {}
    for name in config.domain_dict:
        domain_dict[name] = NCDFDomain(name, config)

    # Assign parent domains
    for domain in domain_dict.values():
        if domain.config.domain_parent is not None:
            domain.parent = domain_dict[domain.config.domain_parent]

    # Domain list that ensures that parent domains are processed before child domains
    domains: List[NCDFDomain] = []

    def add_domain_and_parents(domain: NCDFDomain, domains: List[NCDFDomain]) -> None:
        if domain not in domains:
            if domain.parent is not None:
                add_domain_and_parents(domain.parent, domains)
            domains.append(domain)

    for domain in domain_dict.values():
        add_domain_and_parents(domain, domains)
...
def minimum_terrain_height_global(domains: List[NCDFDomain]) -> float:
...
def process_coordinates(domain: NCDFDomain, zt_min: float) -> None:
    ...
    domain.attributes.origin_x = float(x_UTM_origin[0, 0]) - 0.5 * (
        float(x_UTM_origin[0, 1]) - float(x_UTM_origin[0, 0])
    )
    domain.attributes.origin_y = float(y_UTM_origin[0, 0]) - 0.5 * (
        float(y_UTM_origin[1, 0]) - float(y_UTM_origin[0, 0])
    )
    domain.attributes.origin_lon = float(lon_origin[0, 0]) - 0.5 * (
        float(lon_origin[0, 1]) - float(lon_origin[0, 0])
    )
    domain.attributes.origin_lat = float(lat_origin[0, 0]) - 0.5 * (
        float(lat_origin[1, 0]) - float(lat_origin[0, 0])
    )

    # Read x and y values
    domain.x_UTM.values = domain.read_from_file_1d(domain.input_config.file_x, "x")
    domain.y_UTM.values = domain.read_from_file_1d(
        domain.input_config.file_y, "y", x0=domain.config.y0, x1=domain.config.y1
    )

    # Shift x and y coordinates for x and y coordinates of domain
    domain.x.values = (
        domain.x_UTM.values - min(domain.x_UTM.values.flatten()) + domain.config.pixel_size / 2.0
    )
    domain.y.values = (
        domain.y_UTM.values - min(domain.y_UTM.values.flatten()) + domain.config.pixel_size / 2.0
    )
    ...
    domain.attributes.origin_z = float(zt_min)
    ...
    if domain.config.interpolate_terrain:
        ...
        if domain.parent.x_UTM.values is None or domain.parent.y_UTM.values is None:
        ...
        tmp_x0 = np.searchsorted(domain.parent.x_UTM.values, domain.x_UTM.values[0]) - 1
        tmp_y0 = np.searchsorted(domain.parent.y_UTM.values, domain.y_UTM.values[0]) - 1
        tmp_x1 = np.searchsorted(domain.parent.x_UTM.values, domain.x_UTM.values[-1]) + 1
        tmp_y1 = np.searchsorted(domain.parent.y_UTM.values, domain.y_UTM.values[-1]) + 1
        ...
        if tmp_x1 > domain.x_UTM.values.shape[0]:
            raise Exception(
                f"Parent {domain.parent.name} not fully covering "
                + f"child {domain.name} on the right border"
            )
        if tmp_y0 > domain.y_UTM.values.shape[0]:
            raise Exception(
                f"Parent {domain.parent.name} not fully covering "
                + f"child {domain.name} on the top border"
            )

        tmp_x = domain.parent.x_UTM.values[tmp_x0:tmp_x1]
        tmp_y = domain.parent.x_UTM.values[tmp_y0:tmp_y1]

        zt_parent = domain.parent.zt.values[tmp_y0:tmp_y1, tmp_x0:tmp_x1]

        # Interpolate array and bring to PALM grid of child domain
        zt_ip = interpolate_2d(zt_parent, tmp_x, tmp_y, domain.x_UTM.values, domain.y_UTM.values)
    ...
    # Write CRS (already read in init)
    domain.crs.to_file()


def process_buildings_bridges(
    domain: NCDFDomain, settings: CSDConfigSettings, defaultvalues: Dict[str, float]
) -> None:
...
def process_types(
    domain: NCDFDomain, defaultvalues: Dict[str, float], vt_high_vegetation: List[int]
) -> None:
...
def process_street_type_crossing(domain: NCDFDomain, defaultvalues: Dict[str, float]) -> None:
...
def process_vegetation_roof(domain: NCDFDomain, settings: CSDConfigSettings) -> None:
...
def process_lai(domain: NCDFDomain) -> None:
...
def process_single_trees(
    domain: NCDFDomain, settings: CSDConfigSettings, vt_high_vegetation
) -> None:
...
def process_lai_final(domain: NCDFDomain) -> None:
...
def process_water_temperature(domain: NCDFDomain, defaultvalues: Dict[str, float]) -> None:
...
def consistency_check_update_surface_fraction(domain: NCDFDomain) -> None:
