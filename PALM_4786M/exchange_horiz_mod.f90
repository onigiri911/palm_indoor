...
! Former revisions:
! -----------------
! $Id: exchange_horiz_mod.f90 4626 2020-07-26 09:49:48Z raasch $
! file re-formatted to follow the PALM coding standard
!
! 4474 2020-03-26 09:32:18Z raasch
! bugfix for correct usage of alternative communicators in case of 1d-decompositions and in
! non-parallel mode
!
...
!
!--    Send left boundary, receive right one (synchronous)
       CALL MPI_SENDRECV( ar(nzb,nys_l-nbgp_local,nxl_l),   1, type_yz_int(grid_level), pleft,  0, &
                          ar(nzb,nys_l-nbgp_local,nxr_l+1), 1, type_yz_int(grid_level), pright, 0, &
                          comm2d, status, ierr )
!
!--    Send right boundary, receive left one (synchronous)
       CALL MPI_SENDRECV( ar(nzb,nys_l-nbgp_local,nxr_l+1-nbgp_local), 1, type_yz_int(grid_level), &
                          pright, 1,                                                               &
...
!
!--    Send front boundary, receive rear one (synchronous)
       CALL MPI_SENDRECV( ar(nzb,nys_l,nxl_l-nbgp_local),   1, type_xz_int(grid_level), psouth, 0, &
