...
!--    the number of grid points of an PE, than array p3 will be enlarged.
...
!
!--    Initiation of the multigrid scheme. Does n cycles until the residual is smaller than the
!--    given limit. The accuracy of the solution of the poisson equation will increase with the
!--    number of cycles. If the number of cycles is preset by the user, this number will be carried
!--    out regardless of the accuracy.
       grid_level_count =  0
       mgcycles         =  0
       IF ( mg_cycles == -1 )  THEN
          maximum_mgcycles = 0
...
!
!--    Unset the grid level. Variable is used to determine the MPI datatypes for ghost point
!--    exchange
       grid_level = 0
...
!-- These points are not handled by the above loop. Points may be within buildings, but that
!-- doesn't matter.
    IF ( ibc_p_b == 1 )  THEN
       r(nzb,:,: ) = r(nzb+1,:,:)
    ELSE
...
!--          Use implicit Neumann BCs if the respective gridpoint is inside the building
             rkjim   = r(k,j,i-1) + IBITS( flags(k,j,i-1), 6, 1 ) * ( r(k,j,i) - r(k,j,i-1) )
             rkjip   = r(k,j,i+1) + IBITS( flags(k,j,i+1), 6, 1 ) * ( r(k,j,i) - r(k,j,i+1) )
             rkjpi   = r(k,j+1,i) + IBITS( flags(k,j+1,i), 6, 1 ) * ( r(k,j,i) - r(k,j+1,i) )
             rkjmi   = r(k,j-1,i) + IBITS( flags(k,j-1,i), 6, 1 ) * ( r(k,j,i) - r(k,j-1,i) )
             rkjmim  = r(k,j-1,i-1) + IBITS( flags(k,j-1,i-1), 6, 1 ) * ( r(k,j,i) - r(k,j-1,i-1) )
             rkjpim  = r(k,j+1,i-1) + IBITS( flags(k,j+1,i-1), 6, 1 ) * ( r(k,j,i) - r(k,j+1,i-1) )
             rkjmip  = r(k,j-1,i+1) + IBITS( flags(k,j-1,i+1), 6, 1 ) * ( r(k,j,i) - r(k,j-1,i+1) )
             rkjpip  = r(k,j+1,i+1) + IBITS( flags(k,j+1,i+1), 6, 1 ) * ( r(k,j,i) - r(k,j+1,i+1) )
             rkmji   = r(k-1,j,i) + IBITS( flags(k-1,j,i), 6, 1 ) * ( r(k,j,i) - r(k-1,j,i) )
             rkmjim  = r(k-1,j,i-1) + IBITS( flags(k-1,j,i-1), 6, 1 ) * ( r(k,j,i) - r(k-1,j,i-1) )
             rkmjip  = r(k-1,j,i+1) + IBITS( flags(k-1,j,i+1), 6, 1 ) * ( r(k,j,i) - r(k-1,j,i+1) )
             rkmjpi  = r(k-1,j+1,i) + IBITS( flags(k-1,j+1,i), 6, 1 ) * ( r(k,j,i) - r(k-1,j+1,i) )
             rkmjmi  = r(k-1,j-1,i) + IBITS( flags(k-1,j-1,i), 6, 1 ) * ( r(k,j,i) - r(k-1,j-1,i) )
...
!--          Points in the middle of coarse grid cube
             temp(2*k,2*j+1,2*i+1) = 0.125_wp * ( p(k,j,i)     + p(k,j,i+1)   + p(k,j+1,i)   +     &
                                                  p(k,j+1,i+1) + p(k+1,j,i)   + p(k+1,j,i+1) +     &
...
!
!--          Second, determine if the grid point inside topography is adjacent to a wall and set its
!--          value to a value given by the average of those values obtained from Neumann boundary
!--          condition.
             ...
             wall_total = wall_left + wall_right + wall_south + wall_north + wall_top

             IF ( wall_total > 0.0_wp )  THEN
                p_mg(k,j,i) = 1.0_wp / wall_total * ( wall_left  * p_mg(k,j,i-1) +                 &
...
! Description:
! ------------
!> @todo It may be possible to improve the speed of this routine by using non-blocking communication
!--------------------------------------------------------------------------------------------------!
#if defined( __parallel )
...
!
!--       Set switch for routine exchange_horiz, that ghostpoint exchange has to be carried again
!--       out from now on.
...
!--    Bit 6:  inside building
       ...
       flags = 0
!
!--    In case of masking method, flags are not set and multigrid method works like FFT-solver
       IF ( .NOT. masking_method )  THEN

!
!--       Allocate temporary array for topography heights on coarser grid level. Please note, 2
!--       ghost points are required, in order to calculate flags() on the interior ghost point.
...
!
!--       Exchange ghost points on respective multigrid level. 2 ghost points are required, in order
!--       to calculate flags on.
