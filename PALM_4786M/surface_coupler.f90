...
! Former revisions:
! -----------------
! $Id: surface_coupler.f90 4671 2020-09-09 20:27:58Z pavelkrc $
...
!--       1D surface type to 2D grid.
          CALL transfer_1D_to_2D_equal( surf_def_h(0)%shf, surf_lsm_h(0)%shf,     &
                                        surf_usm_h(0)%shf )
...
!--       from 1D surface type to 2D grid.
          CALL transfer_1D_to_2D_equal( surf_def_h(0)%qsws, surf_lsm_h(0)%qsws,   &
                                        surf_usm_h(0)%qsws )
          IF ( humidity )  THEN
...
!--       transfer from 1D surface type to 2D grid.
          CALL transfer_1D_to_2D_equal( surf_def_h(0)%usws, surf_lsm_h(0)%usws,   &
                                        surf_usm_h(0)%usws )
          CALL MPI_SEND( surface_flux(nysg,nxlg), ngp_xy, MPI_REAL, target_id, &
                         15, comm_inter, ierr )
!
...
!--       transfer from 1D surface type to 2D grid.
          CALL transfer_1D_to_2D_equal( surf_def_h(0)%vsws, surf_lsm_h(0)%vsws,   &
                                        surf_usm_h(0)%vsws )
          CALL MPI_SEND( surface_flux(nysg,nxlg), ngp_xy, MPI_REAL, target_id, &
                         16, comm_inter, ierr )
!
...
!
!--       Transfer from 1D surface type to 2D grid.
          CALL transfer_1D_to_2D_unequal( surf_def_h(0)%shf, surf_lsm_h(0)%shf,   &
                                          surf_usm_h(0)%shf )
...
!
!--       Transfer from 1D surface type to 2D grid.
          CALL transfer_1D_to_2D_unequal( surf_def_h(0)%usws, surf_lsm_h(0)%usws, &
                                          surf_usm_h(0)%usws )
          CALL MPI_REDUCE( total_2d, total_2d_a, ngp_a, MPI_REAL, MPI_SUM, 0, &
                           comm2d, ierr )
...
!------------------------------------------------------------------------------!
        SUBROUTINE transfer_1D_to_2D_equal( def_1d, lsm_1d, usm_1d )
           ...
           IMPLICIT NONE
            ...
            REAL(wp), DIMENSION(1:surf_lsm_h(0)%ns) ::  lsm_1d !< 1D surface flux, natural surfaces
            REAL(wp), DIMENSION(1:surf_usm_h(0)%ns) ::  usm_1d !< 1D surface flux, urban surfaces
!
